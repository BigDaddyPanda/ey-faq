<template>
  <div class="ui container">
    <div class="filter-bar ui basic segment grid">
      ...
      <button class="ui button ml-auto" @click="add_new_modal">Add New Role</button>
    </div>
    <vuetable
      ref="vuetable"
      api-url="http://127.0.0.1:5000/api/role?attributes=id,designation"
      :fields="fields"
    ...
    <modal :show.sync="view_modal" headerClasses="justify-content-center">
      <h4 slot="header" class="title title-up">Role #{{full_focus_data.id}}</h4>
    ...
    <modal :show.sync="edit_modal" headerClasses="justify-content-center">
      <h4 slot="header" class="title title-up">Modify Role</h4>
      <fg-input
        class="col-12"
    ...
    <modal :show.sync="create_modal" headerClasses="justify-content-center">
      <h4 slot="header" class="title title-up">Create Role</h4>
      <fg-input
        class="col-12"
        label="Role Designation"
        placeholder
    ...
    <modal :show.sync="dele_modal" headerClasses="justify-content-center">
      <h4 slot="header" class="title title-up text-danger">Delete Role</h4>
      <h3 class="text-danger">Warning!</h3>
      <h4>You are about to delete a whole Role! Every related user, post, question will be set to Default ones!</h4>
      <template slot="footer">
  ...
  methods: {
    ...
    submit_action(act) {
      if (act == "create") {
        ...
        if (act == "edit") {
          axios
            .put(apiRes("role", this.focus_data.id), {
              designation: this.focus_data.designation
            })
            .then(resp => {
              this.hide_modal();
          ...
          axios.delete(apiRes("role", this.focus_data.id)).then(resp => {
            this.hide_modal();
            nextTick(() => this.$refs.vuetable.refresh());
          });
        }
